...
export class AddTeamCommand extends AppBaseCommand {
  ...
  constructor(commando: CommandoClient) {
    super(commando, {
      ...
      args: [
        {
          key: "osuUsernamesOrIdsOrSeparators",
          prompt: `Enter the osu! usernames or osu ID numbers of the users to be added to a team. Add as many users as you want, each separated by a space. Add multiple teams at once by separating groups of usernames with a "|" character.`,
          type: "string",
          infinite: true
        }
      ]
  ...
  public async run(
    ...
    const createTeamResponse = await this.teamController.create({
      teamDto: {
        osuUsernamesOrIdsOrSeparators: args.osuUsernamesOrIdsOrSeparators
      },
