...
import { TeamLostVirtualMatchGameEvent } from "./game-events/events/team-lost-virtual-match.game-event";
...
export class LeaderboardBuilder {
  ...
  static buildLeaderboard(args: { game: Game; reportables: ReportableContext<ReportableContextType>[] }): ReportableContext<"leaderboard"> {
    ...
    const events = args.reportables
      ...
      .sort((a, b) => (a.item as GameEvent).data.timeOfEvent - (b.item as GameEvent).data.timeOfEvent);

    // team lives
    const teamLives: TeamValue<number> = new Map<number, number>();
    events.forEach(event => {
      if (event.type === "game_event" && event.subType === "team_lost_match") {
    ...
    const lastEvent = events.filter(r => r.subType === "team_lost_match").slice(-1)[0];
  ...
  private static updateTeamScores(
    ...
    (event.item as TeamLostVirtualMatchGameEvent).data.eventMatch.matches.forEach(match => {
  ...
  private static updateTeamLives(
    ...
    const teamId = (event.item as TeamLostVirtualMatchGameEvent).data.teamId;
